<template>
  <div class="app-container">
    ...
    <el-form :inline="true" class="demo-form-inline">    
      <el-form-item>
        <el-input v-model="searchObj.name" placeholder="教师姓名"/>
      </el-form-item>
    ...
    >
      ...
      <el-table-column prop="type" label="职称" align="center" />
      <el-table-column prop="userAccount" label="工号" align="center" />
    ...
    />
    <!-- 添加或修改对话框 -->
    <el-dialog  @open="isDisabled=false" :before-close="onClearFrom" title="新增教师" :visible.sync="dialogVisible" width="30%">
      <el-form :model="form" :rules="rules"  ref="form">
        ...
        <el-form-item label="职称" :label-width="formLabelWidth" prop="type">
        ...
        <el-form-item label="工号" :label-width="formLabelWidth" prop="userAccount">
        <el-input type="text" v-model="form.userAccount" auto-complete="off"></el-input>
        </el-form-item>
...
<script>
import teacherApi from '@/api/system/teacher'
export default {
  data() {
    return {
      typeList:['助教','讲师','副教授','教授'],
  ...
  methods: {
    ...
    updateTop(){
      this.$refs.form.validate(valid=>{
        if (!valid) {
          ...
          this.isDisabled=true //禁用提交按钮
          teacherApi.updateById(this.form)
          .then(response=>{
            this.dialogVisible = false
            this.$message.success(response.message)
            this.fetchData()
    ...
    //列表数据
    fetchData() {
      teacherApi.listPage(this.page,this.limit,this.searchObj)
      .then(response => {
